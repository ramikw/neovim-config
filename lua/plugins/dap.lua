...
return {
	...
	config = function()
		...
		local coreclr_adapter = {
			type = "executable",
			command = custom_function.is_windows() and vim.fn.expand(
	...
	keys = {
		...
		},
		{
			"<F6>",
			function()
				require("vscode-launch").pick_and_run_compound()
			end,
			desc = "Launch VS Code compound",
		},
		{
			"<F7>",
			function()
				require("custom-functions").conditional_breakpoint()
			end,
