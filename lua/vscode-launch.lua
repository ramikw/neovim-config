...
local function run_compound(data, compound)
	...
	if compound.preLaunchTask then
		...
		require("overseer").run_task({ name = compound.preLaunchTask, autostart = false }, function(task, err)
			...
			vim.notify("Waiting for preLaunchTask '" .. compound.preLaunchTask .. "'...", vim.log.levels.INFO)

			vim.defer_fn(function()
				launch_once(
					false,
					("did not finish within %ds (task is still running; open it with :OverseerToggle)"):format(
						M.prelaunch_timeout_ms / 1000
					)
				)
